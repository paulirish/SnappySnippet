...
class DefaultValueFilter {
  ...
  removeDefaultValues(style, tagName, pseudoElement) {
    ...
    const output = {};
    const clone = this.iframe.contentWindow.document.createElement(tagName);
...
class Snapshooter {
  constructor(cssData) {
    ...
    this.shorthandsToCamelCase = this.cssData.cssShorthands.reduce((acc, shorthand) => {
...
function getNonDefaultComputedStyles(element) {
  ...
  const filter = new DefaultValueFilter();
